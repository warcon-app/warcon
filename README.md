...
TRUST_PROXY=true                  # client IPs come from CF-Connecting-IP / X-Real-IP / X-Forwarded-For
```

Only set `TRUST_PROXY` when the proxy is the only way to reach the port; otherwise anyone can spoof
the IP recorded in the audit trail and used for login throttling.
...
| `ORIGIN`                                      | required in production | The exact public URL (scheme, host, port).                                                                |
| `TRUST_PROXY`                                 | `false`                | Take the client IP from forwarded headers.                                                                |
...
cp .env.example .env      # set the two secrets, DATABASE_URL=postgres://warcon:warcon@127.0.0.1:5432/warcon
bun run dev               # http://localhost:5173
bun run check             # svelte-check
bun run build && bun run start   # production build, http://localhost:3000 (needs ORIGIN)
```
...
src/lib/server/auth.ts         Better Auth config (username + admin plugins, Drizzle adapter, PBKDF2 hasher)
...
- Several Warcon replicas can share one database; a Postgres advisory lock makes exactly one of
  them the poller.
- Password hashing is PBKDF2-SHA256, plugged into Better Auth in place of its pure-JS scrypt.
