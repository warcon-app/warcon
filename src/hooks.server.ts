...
import { flag, getEnv, initEnv } from '$lib/server/env';
...
/** Opens the database, applies migrations and starts the analytics poller once per process. */
export const init: ServerInit = async () => {
	...
	if (!env.ENCRYPTION_KEY)
		console.warn('[warcon] ENCRYPTION_KEY is not set; servers cannot be added.');
	startPoller(env);
};
...
export const handle: Handle = async ({ event, resolve }) => {
	...
	// One trustworthy client address for audit rows and Better Auth sessions.
	try {
		event.request.headers.set(
			CLIENT_IP_HEADER,
			resolveClientIp(event.request, event.getClientAddress, flag(env.TRUST_PROXY, false))
		);
	} catch {
		/* immutable headers (should not happen with adapter-node) */
	}
