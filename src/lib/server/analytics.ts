...
export async function loadAnalytics(env: Env, serverId: string, range: Range): Promise<Analytics> {
	...
	const [totals] = await rows<{ n: string; ok: string; peak: number | null; avg: string | null }>(
		db.execute(sql`
	...
	const maps = (
		await rows<{ map: string; n: string; matches: string }>(
			db.execute(sql`
	...
	const hourly = (
		await rows<{ hour: number; avg: string }>(
			db.execute(sql`
