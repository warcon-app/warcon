// An in-Worker imitation of the WARDOGS dedicated server's WDRCON HTTP API.
// Used when a server is added with host "demo" (and ALLOW_DEMO_SERVER is on) so the whole
// panel can be exercised without a game server. State lives in the isolate's memory and
// resets when the isolate is recycled, which is fine for a demo.
//
