...
{#if dialog?.kind === 'edit'}
	...
	<Modal title={d.server ? `Edit ${d.server.name}` : 'Add server'} onclose={() => (dialog = null)}>
		...
		>
			...
			<p class="note">
				The listener must be reachable from Cloudflare: BindAddress 0.0.0.0 (or a reverse proxy /
				Cloudflare Tunnel in front of it). https needs a certificate a browser would trust;
